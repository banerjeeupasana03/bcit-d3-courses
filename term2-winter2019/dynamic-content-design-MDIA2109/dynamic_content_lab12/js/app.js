...
//retrieve pkg from the storage and replace the default
...
//  prox.url = el.value;
  pkg.url = el.value;//backend storage
  //connecting the backend with the frontend (database to the UI)
  //save pkg into your storage
  prox.url = pkg.url;//frontend update(UI)
}
...
function ChangeTitle(el){
  ...
  pkg.title =  el.value;
  prox.title = pkg.title;
}

//FUNCTIONS THAT CHANGES UI
function ChangeImageUI(val) {
  document.querySelector("#previewImg").src = val;
}

function ChangeTitleUI(val){
  document.querySelector("#previewTitle").innerHTML = val;
}
